...
func OutputDemo() {
	...
	go func() {
		...
		msgCh <- console.TaskStateMsg{State: map[int]console.TaskState{
			0: {Status: "[1/4] Fetching dependencies...", StartedAtSec: time.Now().Second()},
		}}
		...
		msgCh <- console.TaskStateMsg{State: map[int]console.TaskState{
			0: {Status: "[2/4] Compiling sources...", StartedAtSec: time.Now().Second()},
		}}
		...
		msgCh <- console.TaskStateMsg{State: map[int]console.TaskState{
			0: {Status: "[3/4] Running tests...", StartedAtSec: time.Now().Second()},
		}}
		...
		time.Sleep(3 * time.Second)
		msgCh <- console.TaskStateMsg{State: map[int]console.TaskState{
			0: {Status: "[4/4] Packaging build artifacts...", StartedAtSec: time.Now().Second()},
		}}
