...
func TestDirectedTargetGraph_SelectTargets(t *testing.T) {
	...
	t.Run("selection with platform skipping", func(t *testing.T) {
		...
		// Call SelectTargets with isTest=false.
		selected, skipped, err := graph.SelectTargets(pattern, []string{testTag}, false)
		if err != nil {
			t.Fatalf("SelectTargets returned unexpected error: %v", err)
		}
		if selected != 1 {
	...
	t.Run("dependency platform mismatch producing an error", func(t *testing.T) {
		...
		// Call SelectTargets.
		_, _, err = graph.SelectTargets(pattern, []string{testTag}, false)
		if err == nil {
	...
	t.Run("filtering by test flag", func(t *testing.T) {
		...
		// When isTest is true, only target5 should be selected.
		selected, skipped, err := graph.SelectTargets(pattern, []string{testTag}, true)
		if err != nil {
			t.Fatalf("SelectTargets returned unexpected error: %v", err)
		}
		if selected != 1 {
		...
		// When isTest is false, only target6 should be selected.
		selected, skipped, err = graph.SelectTargets(pattern, []string{testTag}, false)
		if err != nil {
			t.Fatalf("SelectTargets returned unexpected error: %v", err)
		}
