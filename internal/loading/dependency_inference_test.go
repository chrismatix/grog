...
func TestDependencyInferenceProtocol(t *testing.T) {
	for _, testCase := range []struct {
		...
		// A null package carries no edges, which is what omitting it means too.
		{name: "null package", output: `{"version":1,"packages":{"app":null}}`},
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			err := inferDependencies(t.Context(), packages)
	...
	for _, testCase := range []string{"unknown provider", "duplicate registration", "unused provider", "no providers", "duplicate reference"} {
		t.Run(testCase, func(t *testing.T) {
			...
			case "unused provider", "no providers":
				...
				_, err := os.Stat(filepath.Join(config.Global.WorkspaceRoot, "invoked"))
...
func TestDependencyInferenceConcurrentProvidersMerge(t *testing.T) {
	...
	for _, name := range []string{"first", "second"} {
		contents, err := os.ReadFile(filepath.Join(config.Global.WorkspaceRoot, name))
		require.NoError(t, err)
...
func TestDependencyInferenceGraphErrors(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			_, err = analysis.BuildGraph(nodes)
			require.ErrorContains(t, err, testCase.expectedError)
		})
	}
}
...
func TestDependencyProviderMalformedOutputLimit(t *testing.T) {
	...
	err := inferDependencies(t.Context(), packages)
	require.ErrorContains(t, err, strings.Repeat("x", 2048))
	require.NotContains(t, err.Error(), strings.Repeat("x", 2049))
}
...
targets:
  ...
  - name: explicit
`), 0644))
	loadedPackages, err := LoadAllPackages(t.Context())
	require.NoError(t, err)
...
declare_provider()
`), 0644))
	packageDTO, _, err := (StarlarkLoader{}).Load(t.Context(), buildFile)
	require.NoError(t, err)
