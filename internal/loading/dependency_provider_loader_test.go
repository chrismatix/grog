...
func TestDependencyProviderLoaders(t *testing.T) {
	packageModule, err := filepath.Abs("../../pkl/package.pkl")
...
`, packageModule)},
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			packageDTO, matched, err := testCase.loader.Load(t.Context(), buildFile)
			require.NoError(t, err)
			...
			enrichedPackage, err := getEnrichedPackage(console.GetLogger(t.Context()), ".", packageDTO)
			require.NoError(t, err)
			...
			for _, format := range []string{"json", "yaml"} {
				t.Run(format+" round trip", func(t *testing.T) {
					...
					if format == "json" {
						encoded, err = json.Marshal(packageDTO)
						require.NoError(t, err)
						err = json.Unmarshal(encoded, &decoded)
					} else {
						encoded, err = yaml.Marshal(packageDTO)
						require.NoError(t, err)
...
func TestDependencyProviderEnrichment(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			enrichedPackage, err := getEnrichedPackage(console.GetLogger(t.Context()), "", PackageDTO{DependencyProviders: []*DependencyProviderDTO{&testCase.provider}})
...
func TestDependencyProviderDuplicateLabels(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			_, err := getEnrichedPackage(console.GetLogger(t.Context()), "", combined)
			require.ErrorContains(t, err, "duplicate target label: same")
			for _, reverse := range []bool{false, true} {
				t.Run(fmt.Sprint(reverse), func(t *testing.T) {
					provider, err := getEnrichedPackage(console.GetLogger(t.Context()), "", providerPackage)
					require.NoError(t, err)
					other, err := getEnrichedPackage(console.GetLogger(t.Context()), "", testCase.other)
					require.NoError(t, err)
					if reverse {
						err = mergePackages(provider, other)
					} else {
						err = mergePackages(other, provider)
					}
					require.ErrorContains(t, err, "label: //:same")
				})
			}
		})
	}
