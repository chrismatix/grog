...
- Targets are referenced using [Bazel-style](https://bazel.build/concepts/labels) labels.
- Directory outputs are prefixed with `dir::` to indicate they are directories.
- Cmd are executed as shell scripts in the target's directory.
...
- Any of the target's outputs are not present.
...
Outputs are files or directories that your build command produces. Grog caches these outputs and restores them when possible to avoid unnecessary rebuilds.
