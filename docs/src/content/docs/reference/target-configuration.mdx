...
| `weight`                | `int`                    | Worker slots this target occupies while running (default `1`)         |
...
Optional name of a concurrency group. Targets sharing a group compete for the group's capacity: each running member charges its `weight` against the group, and new members wait when the capacity is reached. Unconfigured groups default to capacity `1` — declaring `concurrency_group = "docker"` on two docker-build targets is enough to serialize them, without any `grog.toml` change.
...
How many worker slots this target occupies while running. Defaults to `1`.

- Always counts against `num_workers` (the global slot pool).
- If `concurrency_group` is set, also counts against the group's capacity.

Set `weight > 1` for commands that are internally parallel — a compiler run with `-j`, a multi-core test suite, `rustc` with codegen-units — so Grog reserves proportional headroom and avoids oversubscribing the machine.

Grog rejects at load time any target whose `weight` exceeds `num_workers` or its group's capacity; those configurations would otherwise block forever.

```pkl
new Target {
  name = "libfoo"
  weight = 4
  command = "make -j4 libfoo"
}
```
