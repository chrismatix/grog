...
| `dependency_providers`  | `label[]`                | Labels of [dependency providers](/topics/dependency-inference) that supply this target's cross-package edges |
...
Labels of [dependency providers](/topics/dependency-inference) — Cargo, uv, npm/pnpm, Go or your own command — that Grog consults at load time for this package's cross-package edges. The inferred labels are merged into `dependencies`, so `cargo_crate(name = "greet")` needs no hand-written `deps` list at all:
...
Naming a provider also registers this target as its package's endpoint for it, which is what other packages' inferred edges resolve to. At most one target per package may register a given provider.

A provider is declared with `dependency_provider` (`dependency_providers:` in YAML and Pkl), takes `name`, `command` and `inputs`, and is **not** a build target: it has no dependencies and no outputs, is never scheduled, and never appears in `grog deps`. Its `inputs` are globs relative to the package it is declared in, which is also the working directory its command runs in and the root the paths it emits are relative to.
