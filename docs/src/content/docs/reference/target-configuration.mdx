...
| `dependency_providers`  | `string[]`               | Names of [dependency providers](/topics/dependency-inference) that supply this target's cross-package edges |
...
Names of [dependency providers](/topics/dependency-inference) — Cargo, uv, npm/pnpm, Go or your own command — that Grog consults at load time for this package's cross-package edges. The inferred labels are merged into `dependencies`, so `cargo_crate(name = "greet")` needs no hand-written `deps` list at all:
...
Naming a provider also registers this target as its package's endpoint for that provider, which is what other packages' inferred edges resolve to. At most one target per package may register a given provider.
