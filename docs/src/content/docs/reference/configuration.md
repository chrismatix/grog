...
- **traces.auto_pull**: When `true`, query commands (`traces list`, `show`, `stats`, `export`) automatically download remote traces to the local cache before querying. This makes traces from CI and other machines visible locally. Defaults to `false`.
