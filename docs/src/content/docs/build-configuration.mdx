...
[Pkl](https://pkl-lang.org/) is a configuration language designed specifically for creating reusable configuration templates. With Pkl, you can:

- Define configuration templates that can be shared within your monorepo or even across the internet
...
One of Pkl's strengths is the ability to create reusable build templates. For example, if you want a standard setup for Next.js projects that includes installing dependencies and building the app, you can create a reusable template:

```pkl
import "package://grog.build/releases/v0.1.4/grog@0.1.4#/package.pkl"

// Create a function that returns a list of targets for a Next.js app
function app(
  inputName: String
) = new Listing<package.Target> {
  // First target: install dependencies
  new {
  ...
  // Second target: build the app (depends on install)
  new {
    name = inputName + "_build"
    command = "npm run build"
    dependencies {
      ":" + inputName + "_install"
    }
