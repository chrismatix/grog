...
To automatically sync remote traces before every query command (`list`, `show`, `stats`, `export`), enable `auto_pull`:
...
With `auto_pull` enabled, running `grog traces list` will first download any new remote trace files, then query across all traces — both local and those from CI or other team members.
...
| `traces.auto_pull` | bool | `false` | Automatically download remote traces before query commands |
