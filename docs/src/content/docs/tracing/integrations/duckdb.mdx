---
title: DuckDB
description: Query Grog execution trace Parquet files directly with DuckDB for fast ad-hoc build analytics.
---

[DuckDB](https://duckdb.org/) is the fastest way to explore [execution trace](/tracing/) data interactively. It reads Parquet files directly — no server, no import, no setup beyond `brew install duckdb`.
