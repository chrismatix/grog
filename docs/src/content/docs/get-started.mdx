...
nix run github:chrismatix/grog

# Install grog together with the Pkl CLI
nix profile install github:chrismatix/grog#grog-with-pkl
```

The flake also exposes `overlays.default` for use in NixOS or home-manager configurations.
...
- uses: chrismatix/setup-grog@v1
  with:
    version: latest # grog release to install
    pkl-version: 0.31.1 # optional, installs the Pkl CLI ("latest" works too)
```
