...
Move `grog.exe` to a directory on `PATH`. Install [Git for Windows](https://gitforwindows.org/) with Git on `PATH`; Grog finds its bundled `sh.exe` automatically. Target commands and resource hooks use POSIX shell syntax. Native `.exe` binaries and shell scripts are supported by `grog run`.

The default cache directory is `%USERPROFILE%\.grog`. Symlink outputs require Windows Developer Mode or administrator privileges. Windows releases support x64; ARM64 is not currently built.
  ...
  "targets": [
    {
      "name": "build_ui",
      "command": "npm run build:ui",
      "inputs": ["frontend/src/**/*.{js,ts,jsx,tsx}", "frontend/package.json"],
    ...
    },
    {
      ...
      "command": "go build -o backend/bin/app ./backend",
      "inputs": ["backend/**/*.go"],
      "outputs": ["backend/bin/app"]
    },
    {
      "name": "build_image",
      "command": "docker build --file Dockerfile --tag myapp .",
      "dependencies": [":build_ui", ":compile_backend"],
      "inputs": ["Dockerfile"],
      "outputs": ["oci::myapp"]
    }
  ]
...
target(
    ...
    inputs = ["Dockerfile"],
    outputs = ["oci::myapp:latest"],
)
```

Save this as `BUILD.star` in your project directory.
...
<Aside>
  You do not have to specify all files that your build target depends on for grog to work. However,
  it is recommended to specify all inputs to avoid unnecessary rebuilds.
</Aside>
...
- [Docker (OCI) Outputs](/topics/docker-outputs) - Learn more about caching Docker / OCI container images
