...
**Grog** is a lightweight build orchestrator that runs your existing commands in parallel—rebuilding only what’s changed—to give small teams instant speedups with zero new DSL.

Unlike some build systems, Grog remains agnostic to the actual build execution and focuses solely on incremental runs, parallel execution, and caching.
