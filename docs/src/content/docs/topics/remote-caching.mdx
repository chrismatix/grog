---
title: Remote Caching (s3, gcs, and azure)
...
Cache object names use the same portable layout for local and remote backends. Keys with characters `* " / \ < > : | ?` will be encoded before storage.
...
To enable remote caching via Azure Blob Storage add the following to your config:
...
1. **Account URL with Azure credentials (recommended):** Set `account_url` and grog will authenticate using `DefaultAzureCredential`, which supports Azure AD, managed identities, and the Azure CLI.
2. **Connection string:** Alternatively, use `connection_string` instead of `account_url` for account-key-based authentication:
