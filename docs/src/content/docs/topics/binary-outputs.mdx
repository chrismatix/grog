...
2. Make it available to dependent target commands via the `$(bin :target_name)` syntax (see [script functions](/topics/script-functions) for details).
...
You can also run multiple binaries at the same time by listing each target
before the `--` separator that introduces the arguments passed to the binaries:

```shell
grog run //tools:api_server //tools:web_dev -- some-arg
```

<Aside>
  Binary outputs are cached just like any other output, which means Grog will avoid rebuilding the
  tool if nothing has changed, making your builds faster.
</Aside>

<Aside>
  If your binary output is a script file, remember to add a
...
<Aside type="caution">
  Specifying either inputs or outputs will cause the target to be rebuilt whenever inputs change
  even when the output checks pass.
</Aside>
...
<Aside type="caution">
  While output checks are convenient, they reduce build reproducibility since they rely on
  inspecting the state of your platform superficially. Consider using binary outputs for critical
  build tools when possible.
</Aside>
...
4. **Consider using a container image** for tools with complex system dependencies (see [Docker (OCI) Outputs](/topics/docker-outputs))
