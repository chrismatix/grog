...
<Aside>
  Files that do not follow the `.grog.sh`/`.grog.py` naming convention will not be discovered
  automatically, but can still be run with `grog run path/to/script` as long as they include a `#
  @grog` metadata block.
</Aside>
...
While this is less convenient and arguably a bit awkward, since there is no actual build step, it can still be useful in case you want to keep your build configuration separator from your scripts.
