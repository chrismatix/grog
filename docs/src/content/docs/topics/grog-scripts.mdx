...
<Aside>
  Files that do not follow the `.grog.sh`/`.grog.py` naming convention will not
  be discovered automatically, but can still be run with `grog run
  path/to/script` as long as they include a `# @grog` metadata block.
</Aside>
