...
Traces are stored using the same [remote caching](/topics/remote-caching/) backends (local filesystem, S3, or GCS) and can be queried from the terminal or exported for use in dashboards like Grafana, Datadog, or Jaeger.
...
When `retention_days` is configured in `grog.toml`, traces older than that threshold are candidates for pruning.

### Storage

Traces use the same `CacheBackend` interface as the build cache. They are stored under a `traces/` prefix with date-based partitioning:

```
traces/
  index                      # lightweight index for fast listing
  data/
    2026-03-30/
      <trace-id>             # full trace data (protobuf)
```

Each trace is typically a few KB, so storage costs are minimal even for large monorepos with frequent builds.
...
| `traces.retention_days` | int | `30` | Number of days to retain traces (0 = unlimited) |
