...
After a target has completed successfully, Grog also records hashes for its declared inputs. A later failure reports files that were added, modified, or removed since that last green run:
