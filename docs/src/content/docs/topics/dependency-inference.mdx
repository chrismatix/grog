...
**Dependency inference** deletes the second copy. A _dependency resolver_ reads the graph your package manager
...
<Aside type="note" title="Inference over-approximates, never under-approximates">
  ...
  the built-in resolvers include it. An extra edge costs you a rebuild. A missing edge costs you correctness.
</Aside>

## Quick start

Two steps. First, declare a resolver in the BUILD file of the directory your workspace is rooted at — for a
Cargo workspace, the repo root. `cargo` is [built in](#built-in-resolvers), so the declaration is a name and a
command:
...
the resolver gets its path dependencies wired automatically:
...
resolver bridges the two by emitting directory-to-directory edges; naming a resolver on a target says "this
target represents my package to that resolver".

The directories a resolver talks about are relative to **the package the resolver is declared in**. Declare
...
1. **Naming a resolver registers the target.** `dependency_resolvers = ["//:cargo"]` marks that target as its
   package's endpoint for that resolver. At most one target per package may register a given resolver.
...
A target may register with several resolvers and receives the union — useful when a package is both a workspace
...
Four resolvers ship with Grog. Write `command = "builtin:<name>"` on a declaration and leave `inputs` off to
...
<Aside type="caution">
  The resolver reads the lockfile, not your imports. A stale `uv.lock` yields a stale graph — keep `uv sync`
...
<Aside type="note" title="A resolver is not a build target">
  ...
  resolver that needs generated code has to generate it itself. That restriction is what keeps `grog deps`,
  `grog changes` and shell completion from ever triggering a build.
</Aside>

<Aside type="tip">
  `dependency_resolvers` at the package level is a list of declarations; on a target it is a list of labels
...
A resolver is any command that prints a JSON document. Nothing else — no plugin API, no linking against Grog.
Implementing one is four steps:

1. Write an executable in whatever language you like that prints the [document](#the-resolver-protocol) to
   stdout.
...
4. **Non-interactive.** stdin is closed. A resolver that prompts hangs every `grog` command, including shell
   completion.
...
<Aside type="tip" title="Cross-language often needs no custom resolver">
  ...
  [uv example](/guides/python) — the language resolver already emits the edge. Reach for a custom resolver only
...
completion. Grog therefore caches resolver output the same way it caches target output.

The cache key is a hash of the protocol version, the resolver's label, its resolved `command`, and the contents
of every file matching `inputs` — resolved by exactly the same globbing that resolves a target's inputs. The
value is the resolver's stdout, stored in the [content-addressable store](/topics/remote-caching), so with a
...
| No resolver referenced | Nothing at all — resolvers only run when a target names them |

Failed runs are never cached, and because the key is content-addressed a stale entry cannot win. Resolver
caching follows the global cache setting, so `--enable-cache=false` re-runs resolvers on every load.

<Aside type="caution" title="Resolvers must be deterministic">
  Sorting handles ordering noise, but a resolver whose _content_ varies between runs — absolute paths,
  timestamps, an unstable upstream tool — will churn the cache key and invalidate targets for no reason.
</Aside>

## Using a committed mapping

If you would rather not run a tool at load time at all — a locked-down CI image, or a resolver that is too slow
...
resolvers avoid. Use it when you need it, not by default.
...
- For a manifest that is _not_ any target's input — most importantly a root workspace file adding or removing
  members — Grog falls back to a conservative rule: if any file matched by a resolver's `inputs` appears in the
  diff, every target registered with that resolver counts as changed.
...
- **Resolvers run before the build, not as part of it.** They cannot depend on build outputs. A resolver that
