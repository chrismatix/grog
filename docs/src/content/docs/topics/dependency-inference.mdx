...
2. Otherwise, if the resolver declared `inputs` for the package, Grog synthesizes a filegroup from them, named
   by the declaration's `synthesized_target` — `_<resolver name>_package` unless set, so `//crates/format:_cargo_package`. Its defining file, for `grog changes` and error
...
| `synthesized_target` | `string`   | Name of the filegroup synthesized for a package that registers no target. Defaults to `_<name>_package`.           |
