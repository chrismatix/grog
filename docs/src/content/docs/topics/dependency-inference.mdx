...
2. Otherwise, if the resolver declared `inputs` for the package, Grog synthesizes a filegroup from them, named
   by the declaration's `generated_target_name` — `_<resolver name>_package` unless set, so `//crates/format:_cargo_package`. Its defining file, for `grog changes` and error
...
Workspace members are the `uv.lock` packages listed in `[manifest] members`; edges are their
`dependencies` and `optional-dependencies` that name other members. Dev groups are excluded for the same reason
as Cargo's `[dev-dependencies]`: uv does not forbid cycles between members, and a test-only edge back to a
dependant is the common way to get one. The root project is not a member: it has no directory of its own.

Each member's synthesized `:_uv` covers `pyproject.toml` and its module tree. With `[tool.uv.build-backend]`
the modules are known exactly (`module-root`, default `src`, and `module-name`, which may be a list). Any other
build backend may package more than one directory, so the inputs are every `.py` and `.pyi` file in the package
plus the full tree of a `src/<name>` or `<name>` directory matching the normalised project name. Package data
outside that tree is not covered — register a filegroup with your own `inputs` for those packages.

Default `inputs`: `uv.lock`, `pyproject.toml`, and every member's `pyproject.toml`, which decides its layout.
...
| `generated_target_name` | `string`   | Name of the filegroup synthesized for a package that registers no target. Defaults to `_<name>_package`.           |
