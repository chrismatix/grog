...
Every package now has a `:_protos` filegroup, and a target that consumes the protos depends on that and nothing
else:
...
`//proto/base:_protos -> //proto/user:_protos -> //proto/order:_protos -> //proto/order:generate`.
