...
<Aside type="note" title="Inference over-approximates, never under-approximates">
  Conditional dependencies — a `cfg(windows)` dependency, an optional feature, an environment marker — are
  included. An extra edge costs a rebuild; a missing edge costs correctness.
</Aside>
...
No `grog.toml`, no script to install, no code generation step. Every crate that names the resolver gets its
path dependencies wired automatically:
...
bridges the two by emitting directory-to-directory edges, and naming a resolver on a target marks that target as
its package's stand-in for the resolver.
...
1. `dependency_resolvers = ["//:cargo"]` registers the target as its package's endpoint for that resolver. At
   most one target per package may register a given resolver.
2. The registered target receives its package's edges as additional `dependencies`, and is what other packages'
   edges point at.
3. Registration, not a naming convention, is the link, so `//crates/format:format`, `//crates/format:sources`
   and `//crates/format:lib` all work. Explicit `dependencies` are kept and merged with the inferred ones.

Every other target in the package already depends on the filegroup, so `:build`, `:test` and `:lint` inherit the
inferred edges untouched.

A target may register with several resolvers and receives the union, which is what a package that is both a
workspace member and a consumer of generated code needs:
...
edges are their `dependencies` and `dev-dependencies` that name other members. Dev dependencies are included
here, since a uv workspace graph is acyclic by construction.

Default `inputs`: `uv.lock`, `pyproject.toml`.

<Aside type="caution">
  The resolver reads the lockfile, not your imports. A stale `uv.lock` yields a stale graph, so keep `uv sync`
  or `UV_LOCKED=1` in CI.
</Aside>
...
Import edges between directories inside your module(s), from `go list`. This is the only built-in that shells
out to its toolchain, so it is slow on a large repo and benefits most from [resolver caching](#caching).
Test-only imports are excluded for the same cycle reason as Cargo's `[dev-dependencies]`.
...
<Aside type="note" title="A resolver is not a build target">
  Declarations are loaded, never scheduled. A resolver has no `dependencies` and no `outputs`, `grog build
  //...` does not build it, and it never appears in `grog deps`. It cannot consume another target's output; one
  that needs generated code has to generate it itself. That is what keeps `grog deps`, `grog changes` and shell
  completion from triggering a build.
</Aside>
...
6. **Complete.** Report every edge whose change should rebuild the importing package, including conditional
   ones. An omitted edge is the silent staleness inference exists to remove, and nothing downstream can detect
   it — the built-in resolvers over-approximate for this reason, and a resolver you write should too.
...
[`examples/codegen`](https://github.com/chrismatix/grog/tree/main/examples/codegen). A resolver that maps each
consumer directory to the codegen target is about a dozen lines:

```python
#!/usr/bin/env python3
# tools/grog/proto_resolver.py
import json
import sys
...
<Aside type="tip" title="Cross-language often needs no custom resolver">
  If the generated package is also a workspace member, as `lib/proto` is in the [uv example](/guides/python),
  the language resolver already emits the edge. Write a custom resolver only when no package manager spans the
  boundary.
</Aside>
...
The cache key hashes the protocol version, the resolver's label, its resolved `command`, and the contents of
every file matching `inputs`, globbed exactly as a target's inputs are. The value is the resolver's stdout,
...
| No resolver referenced | Nothing; resolvers only run when a target names them |
...
| `resolver //:cargo reports crates/greet depends on crates/format, which has no target registered for //:cargo` | A workspace member with no BUILD file, or one whose target does not name the resolver. Add the field, or remove the crate from the workspace. |
...
A workspace member with no registered target is ignored rather than an error, since a repo may contain packages
Grog does not build. It is an error only when a package Grog does build depends on one.
...
- **Dev dependencies and test-only imports are excluded** by the `cargo` and `go` resolvers. Both ecosystems
  permit cycles there, and inferred edges land on the filegroup every target depends on, so importing them would
...
- **A package with no registered target cannot be depended on.** It is ignored while nothing points at it, and a
  load error as soon as a participating package does. Letting a resolver declare a package's `inputs` so Grog can
  synthesize the missing filegroup is the planned fix; until then, register a target or drop the package from
  the workspace.
