...
Workspace members are the `uv.lock` packages whose source is `editable`, `directory` or `virtual`; edges are their
...
Each member's synthesized `:_uv` covers `pyproject.toml` and its module tree. The module comes from
`[tool.uv.build-backend]` (`module-root`, default `src`, and `module-name`, which may be a list), else from a
`src/<name>` or `<name>` directory matching the normalised project name; a member with neither gets every
`.py` and `.pyi` file. Package data outside the module tree is not covered — register a filegroup with your own
`inputs` for those packages.
