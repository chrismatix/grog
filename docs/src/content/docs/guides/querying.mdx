...
```shell
# Build all targets in the `model_training` package that have at least one of the tags
...
`grog deps <target-pattern> [--transitive]`

Lists all the dependencies listed by a target.
...
# Returns all build targets that have any of the go files as their input
```
